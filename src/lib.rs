...
pub struct RemoveMultiMap<K, V> {
    key: K,
    value: V,
}

impl<K, V: 'static> actix::Message for RemoveMultiMap<K, V> {
    type Result = Option<V>;
}
...
pub fn remove_multimap<K, V>(k: K, v: V) -> RemoveMultiMap<K, V> {
    RemoveMultiMap { key: k, value: v }
}
